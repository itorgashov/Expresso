...
| Situation | Exception |
|---|---|
| `query` or the field-catalog array is `null` | `ArgumentNullException` |
...
**Note on reflection wrapping:** comparison functions (`eq`, `neq`, `gt`, `gte`, `lt`, `lte`) and arithmetic functions (`abs`, `add`, `sub`, `mult`, `div`) are constructed by the parser via reflection (`Activator.CreateInstance`). If the expression-tree constructor itself rejects the arguments (see the next section), the resulting exception is wrapped in `System.Reflection.TargetInvocationException` — inspect `.InnerException` to get the real `ArgumentException`/`ArgumentNullException`. String functions, logical functions (`and`/`or`/`not`), `in`, and `isnull` are constructed directly and do **not** get wrapped.
...
| Situation | Exception |
|---|---|
| `filterCriteria`, `sortDirective`, `fieldToColumnMap`, or `paramNamePrefix` is `null` | `ArgumentNullException` |
...
A missing field-to-column mapping should not normally occur if `fieldToColumnMap` is kept in sync with your `IRequestFieldsInfoProvider` allow-list — see [docs/field-providers.md](field-providers.md).
