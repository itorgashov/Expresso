...
namespace Expresso.Tests.Core.CriteriaExpressions
{
    public class GteFuncTests
    {
        ...
        public void NumLtFunc_ValidOperands_SetsArguments(Type leftType, Type rightType)
        {
            var left = new MockExpressionOfType(leftType);
            var right = new MockExpressionOfType(rightType);

            var func = new GteFunc(left, right);
        ...
        public void NumLtFunc_InvalidOperands_ThrowsArgumentException(Type leftType, Type rightType)
        {
            var left = new MockExpressionOfType(leftType);
            var right = new MockExpressionOfType(rightType);

            Assert.Throws<ArgumentException>(() => new GteFunc(left, right));
        }

        [Fact]
        public void NumLtFunc_NullLeftOperand_ThrowsArgumentNullException()
        {
            var right = new MockExpressionOfType(typeof(int));

            Assert.Throws<ArgumentNullException>(() => new GteFunc(null!, right));
        }

        [Fact]
        public void NumLtFunc_NullRightOperand_ThrowsArgumentNullException()
        {
            var left = new MockExpressionOfType(typeof(int));

            Assert.Throws<ArgumentNullException>(() => new GteFunc(left, null!));
        }
        ...
        public void Equals_ReturnsTrue_ForSameOperands()
        {
            ...
            GteFunc fn1 = new(left, right);
            GteFunc fn2 = new(left, right);
